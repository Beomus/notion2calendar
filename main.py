...
class GCalendar:
    ...
    def add_event(self, events, calendarId='primary'):
        ...
        if not type(events) is list:
            raise TypeError("Events should be in a list (even single event), expecting each event "
    ...
    def update_event(self, events, calendarId, Id):
        ...
        for event in events:
            event_result = self.service.events().update(calendarId=calendarId, eventId=Id, body=event).execute()
            print(f"[INFO]: Event created")
            print(f"ID: {event_result['id']} | HTML: {event_result.get('htmlLink')}")
    ...
    def delete_events(self, Ids, calendarId='primary'):
        ...
        for Id in Ids:
            ...
            try:
                self.service.events().delete(calendarId=calendarId, eventId=Id).execute()
...
def mds_to_events(path, date_time):
    md_list = [i for i in os.listdir(path) if os.path.isfile(os.path.join(path, i)) and i[-3:] == '.md']
...
if __name__ == '__main__':
    root_path = os.path.join('notion_calendar', 'Workout Calendar 897ed488b6f94fa4bd29eddee342dd9a')
